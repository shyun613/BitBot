#!/usr/bin/env python3
"""A1 선물 백테스트 (V21 logic + BTC/ETH/SOL universe) (ENS_fut_L3_k3_12652d57, 2026-04-17 확정).

실거래 SSOT: trade/auto_trade_binance_a1.py STRATEGIES (V21과 동일 로직, 유니버스만 BTC/ETH/SOL).
...
from a1_backtest import load_data, run
...
# A1 STRATEGIES — V21 STRATEGIES와 완전 동일, 유니버스만 a1_backtest에서 BTC/ETH/SOL로 고정
A1_STRATEGIES = {
    '4h_S240_SN120': dict(
...
A1_COMBO = {k: 1/3 for k in A1_STRATEGIES}
...
def main():
    t0 = time.time()
    print(f'A1 백테스트 ({START} ~ {END})')
    print(f'  전략: {list(A1_STRATEGIES)}')
    print(f'  가중: EW 1/3 each, 고정 3x, 스탑/가드 없음')
    print()
    print('Loading data...')
    intervals = sorted({cfg['interval'] for cfg in A1_STRATEGIES.values()} | {'1h'})
    ...
    traces = {}
    for name, cfg in A1_STRATEGIES.items():
        t1 = time.time()
        traces[name] = generate_trace(data, cfg)
        print(f'  {name}: {len(traces[name])} rebal points ({time.time()-t1:.1f}s)')

    combined = combine_targets({k: traces[k] for k in A1_COMBO}, A1_COMBO, all_dates)

    print('Running A1 single-account engine (fixed 3x, no stop, no gate)...')
