#!/usr/bin/env python3
"""1h봉 Coarse Grid — 캐스케이드 STEP 4.

극단기(시간 단위)부터 D수준 장기까지 전 범위 탐색.
현물 + 선물(5x). 멀티프로세싱 + 체크포인트 지원.
"""
...
# ─── 1h봉 Coarse Grid (바 단위, 24봉/일) ───────────────────
# 로그스케일 진행, 봉 단위 기준
SMA_LIST = [12, 24, 48, 120, 240, 480, 720, 1440, 2880, 4800]       # 0.5일~200일
MS_LIST = [1, 2, 3, 6, 12, 20, 24, 48, 120, 360, 720, 1920]         # 1h~80일
ML_LIST = [72, 240, 480, 960, 1440, 2880, 4320, 5760, 8640]         # 3일~360일
SNAP_LIST = [72, 168, 336, 720, 1440, 2880, 4320]                   # 3일~180일
VC_LIST = [0.05, 0.07]
...
def main():
    ...
    t0 = time.time()
    combos = build_combos()
    print(f"1h Coarse Grid: {len(combos)} 조합 x {len(MODES)} 모드 ({args.workers} workers)")

    print("데이터 로딩...")
    prices, um, funding = load_binance_prices('1h', top_n=TOP_N)
    print(f"  1h: {len(prices)} coins, 완료 ({time.time()-t0:.1f}s)\n")

    for mode, leverage, tx_cost, label in MODES:
        start, end = PERIOD
        json_path = f'research/grid_results/1h_coarse_{label.lower()}.json'
