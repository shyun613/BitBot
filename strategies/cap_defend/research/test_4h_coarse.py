#!/usr/bin/env python3
"""4h봉 Coarse Grid — 캐스케이드 STEP 2.

D봉과 독립적인 넓은 파라미터 공간 탐색. 현물 + 선물(5x).
멀티프로세싱 + 체크포인트 지원.
"""
...
# ─── 4h봉 Coarse Grid (바 단위, 6봉/일) ────────────────────
# 로그스케일 진행, 봉 단위 기준
SMA_LIST = [12, 30, 60, 120, 180, 240, 360, 480, 720, 1200]  # 2일~200일
MS_LIST = [1, 3, 6, 12, 20, 30, 60, 120, 240, 360, 480]       # 4h~80일
ML_LIST = [18, 60, 120, 240, 360, 720, 1080, 1440, 2160]      # 3일~360일
SNAP_LIST = [42, 84, 168, 360, 540, 720, 1080]                # 7일~180일
VC_LIST = [0.05, 0.07]
...
def main():
    ...
    t0 = time.time()
    combos = build_combos()
    print(f"4h Coarse Grid: {len(combos)} 조합 x {len(MODES)} 모드 ({args.workers} workers)")

    print("데이터 로딩...")
    prices, um, funding = load_binance_prices('4h', top_n=TOP_N)
    print(f"  4h: {len(prices)} coins, 완료 ({time.time()-t0:.1f}s)\n")

    for mode, leverage, tx_cost, label in MODES:
        start, end = PERIOD
        json_path = f'research/grid_results/4h_coarse_{label.lower()}.json'
