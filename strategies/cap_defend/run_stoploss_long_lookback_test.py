...
INPUT_CSV = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'stoploss_cooldown_results.csv')
OUTPUT_CSV = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'stoploss_long_lookback_results.csv')
LONG_LOOKBACKS = [48, 72, 96, 168, 336]
...
def load_candidates(top_n):
    ...
    with open(INPUT_CSV, newline='') as f:
        ...
        for row in reader:
            for k in ['leverage', 'stop_pct', 'Sharpe', 'CAGR', 'MDD', 'Cal']:
                row[k] = float(row[k])
            for k in ['stop_lookback_bars', 'reentry_cooldown_bars', 'Liq', 'Stops', 'Rebal']:
                row[k] = int(float(row[k]))
            if not row['stop_kind'].startswith('rolling'):
                continue
            rows.append(row)
    ...
    for lev in sorted({r['leverage'] for r in rows}):
        ...
        sub.sort(key=lambda r: (-r['Cal'], r['stop_kind'], r['stop_lookback_bars'], r['reentry_cooldown_bars']))
        seen = set()
        count = 0
        for r in sub:
            key = (r['stop_kind'], r['stop_pct'], r['reentry_cooldown_bars'])
            if key in seen:
                continue
            seen.add(key)
            candidates.append({
                ...
                'stop_pct': r['stop_pct'],
                'reentry_cooldown_bars': r['reentry_cooldown_bars'],
                'base_lookback_bars': r['stop_lookback_bars'],
            })
            count += 1
            if count >= top_n:
                break
...
def _run_case(item):
    run_idx, total_runs, cand, lookback = item
    engine = SingleAccountEngine(
        ...
        stop_lookback_bars=lookback,
        reentry_cooldown_bars=cand['reentry_cooldown_bars'],
    )
    ...
    row = {
        ...
        'stop_kind': cand['stop_kind'],
        'stop_pct': cand['stop_pct'],
        'base_lookback_bars': cand['base_lookback_bars'],
        'test_lookback_bars': lookback,
        'reentry_cooldown_bars': cand['reentry_cooldown_bars'],
    ...
    progress = (
        ...
        f"base_lb={cand['base_lookback_bars']} test_lb={lookback} cd={cand['reentry_cooldown_bars']} "
...
def main():
    parser = argparse.ArgumentParser(description='쿨다운 상위 rolling 후보 대상 긴 룩백 테스트')
    ...
    print(f'Loaded rolling candidates: {len(candidates)}')
    ...
    for cand in candidates:
        lookbacks = sorted(set([cand['base_lookback_bars']] + LONG_LOOKBACKS))
        for lb in lookbacks:
            run_idx += 1
            work_items.append((run_idx, 0, cand, lb))
    total_runs = len(work_items)
    work_items = [(idx, total_runs, cand, lb) for idx, _, cand, lb in work_items]
    ...
    rows.sort(key=lambda r: (r['leverage'], -r['Cal'], r['stop_kind'], r['test_lookback_bars']))
    fieldnames = [
        'leverage', 'stop_kind', 'stop_pct', 'base_lookback_bars', 'test_lookback_bars', 'reentry_cooldown_bars',
    ...
    for lev in sorted({r['leverage'] for r in rows}):
        ...
        for r in top:
            print(
                f"{r['stop_kind']:<30} pct={r['stop_pct']:.0%} "
                f"base_lb={r['base_lookback_bars']:>3} test_lb={r['test_lookback_bars']:>3} "
